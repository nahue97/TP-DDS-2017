...
public class AppData {
	...
	public void cargarEmpresasDeCuentas(PathFile datosDeCarga) {
		...
		providersCuenta.forEach(
				proveedor -> proveedor.getInformationCuentas(datosDeCarga).forEach(c -> empresas.add(c.getEmpresa())));
	...
	private void inicializarIndicadoresCalculados(){
		List<IndicadorCalculado> indicadoresCalculados = new ArrayList<>();
		
	    List<Empresa> empresasACalcular = RepositorioCuentas.getInstance().getEmpresasConCuenta();
	    List<Indicador> indicadoresACalcular = RepositorioIndicadores.getInstance().getAll();
        ...
        indicadoresACalcular.forEach(ind -> empresasACalcular.forEach(emp -> periodosACalcular.forEach(per -> indicadoresCalculados.add(new IndicadorCalculado(ind, emp, per))))); 

	    indicadoresCalculados.forEach(indicadorCalculado -> RepositorioIndicadoresCalculados.getInstance().add(indicadorCalculado));
	}

	public void inicializarMetodologias() {
		ReglaComparativa regla1 = new ReglaComparativa("Regla1", RepositorioIndicadores.getInstance().getAll().get(0),
				Criterio.MAYOR);
		ReglaComparativa regla2 = new ReglaComparativa("Regla2", RepositorioIndicadores.getInstance().getAll().get(1),
				Criterio.MAYOR);
		ReglaComparativa regla3 = new ReglaComparativa("Regla3", RepositorioIndicadores.getInstance().getAll().get(2),
				Criterio.MAYOR);
		ReglaComparativa regla4 = new ReglaComparativa("Regla4", RepositorioIndicadores.getInstance().getAll().get(3),
				Criterio.MAYOR);

		ReglaTaxativa regla5 = new ReglaTaxativa("Regla5", RepositorioIndicadores.getInstance().getAll().get(4), '>',
				new BigDecimal(170));
		ReglaTaxativa regla6 = new ReglaTaxativa("Regla6", RepositorioIndicadores.getInstance().getAll().get(5), '<',
				new BigDecimal(2900));
		ReglaTaxativa regla7 = new ReglaTaxativa("Regla7", RepositorioIndicadores.getInstance().getAll().get(6), '>',
				new BigDecimal(260));
		ReglaTaxativa regla8 = new ReglaTaxativa("Regla8", RepositorioIndicadores.getInstance().getAll().get(7), '>',
				new BigDecimal(10));

		ReglaComparativa regla9 = new ReglaComparativa("Regla9", RepositorioIndicadores.getInstance().getAll().get(2),
				Criterio.MAYOR);
		ReglaComparativa regla10 = new ReglaComparativa("Regla10", RepositorioIndicadores.getInstance().getAll().get(3),
				Criterio.MAYOR);
		ReglaTaxativa regla11 = new ReglaTaxativa("Regla11", RepositorioIndicadores.getInstance().getAll().get(4), '>',
				new BigDecimal(170));
		ReglaTaxativa regla12 = new ReglaTaxativa("Regla12", RepositorioIndicadores.getInstance().getAll().get(5), '<',
				new BigDecimal(2900));
		...
		Metodologia metodologia3 = new Metodologia("Metodologia Mazzeo", reglas3,
				RepositorioUsuarios.getInstance().getAll().get(0));
